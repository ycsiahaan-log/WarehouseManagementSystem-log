...
function getTextSkip() {
  var url = UrlApi + '/nug_api/pickingResi?noso=' + so;
  ...
  fetch(url, {
    ...
    .then((data) => {
      console.log(data);
      var getData2 = data.data.rows;
      jmlItem = getData2[0]['jmlitem'];
      jumlahorder = getData2[0]['jumlahorder'];
      ...
      alert('Ada Barang SKIP');
      getNextLocation();
    })
    .catch((error) => {
      // Menangani kesalahan
      document.getElementById('btnStock').disabled = false;
      alert('Request Gagal. (POST:/nug_api/pickingResi)');
